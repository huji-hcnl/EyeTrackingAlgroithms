...
from Utils.visual_angle_utils import pixels_to_visual_angles
...
class NHDetector(BaseDetector):
    ...
    def _identify_gaze_event_candidates(self, x: np.ndarray,
        ...
        print(velocity_peak_threshold)
        above_threshold_indexes = np.where(angular_velocities > velocity_peak_threshold)[0]
        velocity_peaks_indexes = argrelextrema(angular_velocities[above_threshold_indexes], np.greater)[0]
        # print(velocity_peaks_indexes)
        ...
        # candidates = NHDetector._glissade_detection(angular_velocities, timestamps, candidates, saccade_peaks_list,
        #                                             saccade_dict)
        # # step 5 - fixation detection
        # candidates = self._fixation_detection(candidates, timestamps)
        #
    ...
    def _filter_and_denoise(self, x, y, view_dist, pixel_size, timestamps):
        ...
        visual_angles = pixels_to_visual_angles(x_filtered, y_filtered, view_dist, pixel_size)
        ...
        # angular_velocities = np.gradient(visual_angles, dt)
        # angular_accelerations = np.gradient(angular_velocities, dt)
        # # denoise high values
    ...
    def _find_onset_saccade(self, peak_index, angular_velocities, prior_offset):
        # saccade will either begin at the beginning of the array, or at the end of another saccade
        onset_boundry = max(0, prior_offset)
        ...
        while onset_boundry < saccade_onset_index:
            # if local minimum
            if angular_velocities[saccade_onset_index - 1] > angular_velocities[saccade_onset_index] and \
                ...
                below_current_sample = angular_velocities[angular_velocities < angular_velocities[saccade_onset_index]]
                current_onset_threshold = np.mean(below_current_sample) + 3 * np.std(below_current_sample)
        ...
        return saccade_onset_index

    def _find_offset_saccade(self, peak_index, angular_velocities, timestamps, saccade_onset_index):
        ...
        while saccade_offset_index < len(angular_velocities) - 1:
            ...
            if angular_velocities[saccade_offset_index - 1] > angular_velocities[saccade_offset_index] and \
                ...
                window_ending_index = NHDetector._find_window_end_index(saccade_offset_index, 40, timestamps)
                ...
                saccade_offset_threshold = self._alpha * angular_velocities[saccade_onset_index] + self._beta * \
    ...
    def _saccade_detection(self, velocity_peaks_indexes, angular_velocities, timestamps, candidates):
        ...
        for peak_index in velocity_peaks_indexes:
            saccade_onset_index = self._find_onset_saccade(peak_index, angular_velocities, prior_offset)
            saccade_offset_index, saccade_offset_threshold = self._find_offset_saccade(peak_index, angular_velocities,
                                                                                       timestamps, saccade_onset_index)
            prior_offset = saccade_offset_index
    ...
    def _find_window_end_index(window_start_index, time_window_length, timestamps):
        window_indexes = np.where(np.cumsum(timestamps[window_start_index:]) >= time_window_length)[0]
    ...
    def _glissade_detection(angular_velocities, timestamps, candidates, saccade_peaks_list, saccade_dict):
        ...
        for i, saccade_peak in enumerate(saccade_peaks_list):
            ...
            window_ending_index = NHDetector._find_window_end_index(glissade_onset, 40, timestamps)
