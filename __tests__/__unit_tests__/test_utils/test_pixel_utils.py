...
class TestPixelUtils(unittest.TestCase):
    ...
    def test_calculate_velocities(self):
        ...
        self.assertTrue(np.array_equal(np.array([np.nan, sqrt2, sqrt2, sqrt2, sqrt2]) * cnst.MILLISECONDS_PER_SECOND,
                                       pixel_utils.calculate_velocities(x_coords, y_coords1, t_coords1),
                                       equal_nan=True))
        y_coords2 = np.zeros_like(x_coords)
        self.assertTrue(np.array_equal(np.array([np.nan, 1, 1, 1, 1]) * cnst.MILLISECONDS_PER_SECOND,
                                       pixel_utils.calculate_velocities(x_coords, y_coords2, t_coords1),
                                       equal_nan=True))
        t_coords2 = t_coords1 * 2
        self.assertTrue(np.array_equal(np.array([np.nan, 0.5, 0.5, 0.5, 0.5]) * cnst.MILLISECONDS_PER_SECOND,
                                       pixel_utils.calculate_velocities(x_coords, y_coords2, t_coords2),
                                       equal_nan=True))
        y_coords3 = y_coords1.copy().astype(float)
        y_coords3[2] = np.nan
        self.assertTrue(np.array_equal(np.array([np.nan, sqrt2, np.nan, np.nan, sqrt2]) * cnst.MILLISECONDS_PER_SECOND,
