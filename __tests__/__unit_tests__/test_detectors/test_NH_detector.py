...
class TestNHDetector(unittest.TestCase):
    ...
    def test_NH(self):
        ...
        pixel_size = 0.037824
        part_data = data
        candidates = [GazeEventTypeEnum.UNDEFINED] * len(part_data["label"])
        labels = np.array(part_data["label"])
        x_coords = np.array(part_data["right_eye_x"])
        y_coords = np.array(part_data["right_eye_y"])
        timestamps = np.array(part_data["time"])
        ...
        # candidates = detect_obj._identify_blink_candidates(x_coords, y_coords, np.array(candidates))[2]
